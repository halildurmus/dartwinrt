// Copyright (c) 2023, Dart | Windows. Please see the AUTHORS file for details.
// All rights reserved. Use of this source code is governed by a BSD-style
// license that can be found in the LICENSE file.

// THIS FILE IS GENERATED AUTOMATICALLY AND SHOULD NOT BE EDITED DIRECTLY.

// ignore_for_file: deprecated_member_use_from_same_package

import 'package:windows_foundation/windows_foundation.dart';

/// Specifies the result of a MediaFrameSourceController.GetPropertyAsync or
/// MediaFrameSourceController.GetPropertyByExtendedIdAsync operation.
enum MediaFrameSourceGetPropertyStatus implements WinRTEnum {
  success(0),
  unknownFailure(1),
  notSupported(2),
  deviceNotAvailable(3),
  maxPropertyValueSizeTooSmall(4),
  maxPropertyValueSizeRequired(5);

  @override
  final int value;

  const MediaFrameSourceGetPropertyStatus(this.value);

  factory MediaFrameSourceGetPropertyStatus.from(int value) =>
      MediaFrameSourceGetPropertyStatus.values.firstWhere(
          (e) => e.value == value,
          orElse: () => throw ArgumentError.value(
              value, 'value', 'No enum value with that value'));
}
